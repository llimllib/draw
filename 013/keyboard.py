...
# ideas:
# - figure out how to sync with clock on live instruments orchestra, display clock
# - get autoreload working
# - get drums working
# - show notes in time, repeating
# - make cool noises
# - figure out the synatx around named instruments
...
class KeyboardSynth(pyglet.window.Window):
    ...
    def __init__(self, csound):
        ...
        # the file to load an orchestra from
        self.csound_file = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "a.csd")
        )
        self._load_csound()

        # the total time since we last checked to see if a.csd was updated
        self.fcheck_time = 0
        self.updated_csound = os.stat("a.csd").st_mtime
    ...
    def _load_csound(self):
        # XXX: could I just load the orchestra instead of stopping the thread,
        # re-compiling, and restarting? For now, let's just go nuclear I guess,
        # if it works then :shrug:
        # https://csound.com/docs/ctcsound/ctcsound-PT.html

        print("---------------loading csound")

        # if we already have a thread, and it hasn't been stopped, quit
        if hasattr(self, "t") and self.t.isRunning():
            raise Exception("performance thread needs to be stopped to load a new one")

        orc = open(self.csound_file).read().format(SAMPLERATE=SAMPLERATE)
    ...
    def on_midi(self, msg):
        if msg.type == "clock":
            ...
            if msg.channel == self.KEYBOARD:
                ...
                o = f'schedule(nstrnum("asub") + {msg.note / 100:.2}, 0, -1, {notes[msg.note][1]}, {msg.velocity/255:.2})'
                print(o)
                self.cs.compileOrc(o)
                # cmsg = f'i "asub"{msg.note} 0 -1 {notes[msg.note][1]} {msg.velocity/255:.2}'
                # print(cmsg)
                # self.t.inputMessage(cmsg)
            if msg.channel == self.DRUMPAD:
                print(msg)
                cmsg = f'i "click2" 0 -1 6000 {msg.velocity/127:.2}'
                print(cmsg)
                self.t.inputMessage(cmsg)
    ...
    def update(self, dt):
        ...
        # check for file update no sooner than 1/10 of a sec
        self.fcheck_time += dt
        if self.fcheck_time > 0.1:
            self.fcheck_time = 0
            mtime = os.stat("a.csd").st_mtime
            if mtime > self.updated_csound:
                self.updated_csound = mtime
                self._load_csound()
